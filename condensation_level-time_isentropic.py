...
ds = ds.where(ds.level<=400, drop=True)
ds = ds.sel(lat=lat, method='nearest')
ds = ds.sel(lon=lon, method='nearest')
ds = ds.reset_coords('lat', drop=True).reset_coords('lon', drop=True)
...
cf = ds.PV_lait.plot.contourf(ax=ax,  x='level', y='time', cmap='OrRd', levels=21)
...
cf1 = ds.cond_loc_fill.plot.contour(ax=ax, x='level', y='time', levels=[0.5], colors='black', linewidths=1)
ax.set_yticks(tick_time, tick_ls)
plt.savefig(f'{path}/Plots/level-time_condloc_{lon}deg_my{year}_Ls{Ls_early}-{Ls_late}_{latint}-{latdec}deg.pdf')