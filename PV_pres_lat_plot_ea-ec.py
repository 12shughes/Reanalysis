...
while i<=5:
    ...
    r = np.remainder(i, 2)
    if r == 0:
        type = 'EMARS_data/Analysis/'
        dataset = 'emars'
        title = 'EMARS Analysis'
    elif r == 1:
        type = 'EMARS_data/Control/'
        dataset = 'emars'
        title = 'EMARS Control'
    path = '/disco/share/sh1293/%s/' %(type)
    if c == 0:
        my = 24
    elif c == 1:
        my = 25
    ...
    dataisen = xr.open_dataset(path + '/Isentropic/isentropic_%s_my%d.nc' %(dataset, my))
    data = xr.open_dataset(path + '/Isobaric/isobaric_%s_my%d.nc' %(dataset, my))
...
plt.savefig('/disco/share/sh1293/EMARS_data/pres_all_data_grid_PV_Ls%d-%d.pdf' %(Lsmin, Lsmax))
